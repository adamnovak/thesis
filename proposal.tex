...
Suffix arrays can be used as indices to speed up substring search on the string they are derived from. Because a suffix array is equivalent to a sorted list of all the suffixes in a string, and because every substring of the string is at the beginning of some suffix, a simple binary search is sufficient to find (at least one instance of) any substring that is present. Supplementing the suffix array with a longest common prefix array, holding the length of the longest common prefix of adjacent suffixes, can further speed up this search, by requiring only a single-character comparison (instead of a string comparison) at each search step \cite{manber1993suffix}.
...
Since a BWT sorts all rotations of the string being transformed, the BWT of a string implies its suffix array. 
