...
\subsection{Interactive Distributed Computing with Apache Spark}
...
\subsection{API Description with Avro}
...
\subsection{NOTCH2NL and Sequence Ambiguity}
\label{subsec:notch}
...
\subsection{Project \#1: Tumor Map Visualization}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/01_introduction/tumormap.png}
...
\subsection{Project \#2: NOTCH2NL Copy Number Variation Analysis}

My graph-based analysis of NOTCH2NL paralog copy number is a second and somewhat more relevant piece of prior work. As discussed in Subsection~\ref{subsec:notch}, while the older GRCh37 human genome assembly had only a single NOTCH2NL gene annotated, new information from sequencing haploid cells (from a hyatidiform mole) revealed the existence of four paralogs, here dubbed NOTCH2NL-A, NOTCH2NL-B, NOTCH2NL-C, and NOTCH2NL-D \cite{jacobs2014recently}. Moreover, NOTCH2NL-A, -B, and -C were found to be located quite close to ``the 1q21.1 distal deletion/duplication locus'' \cite{jacobs2014recently}, copy number variations of which have been associated with congenital microcephaly and macrocephaly \cite{jacobs2014recently}. The goal of our project was to determine if deletions or duplications of any of the nearby NOTCH2NL genes could potentially explain these microcephalic and macrocephalic phenotypes.
...
\begin{figure}[h!t]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/01_introduction/notch2nl.png}
...
\subsection{Project \#3: Mapping to a Reference Genome Structure}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/01_introduction/lrexactmatch.png}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/01_introduction/lrexactmatchgraph.png}
...
\subsection{Specific Aim \#1: Develop Reference Hierarchy Theory}
...
\subsubsection{Define a Practical Merging Scheme}
\label{subsubsec:aim1merging}
...
To complete this part of the project, I will need to define a mathematical model of the greedy merging scheme in the language of reference structure theory, and prove its essential properties. I will also need to demonstrate that it is practical to apply it at the scale necessary to complete Specific Aim \#2 (Section~\ref{subsec:aim2}), through prototyping.

\subsubsection{Define a Practical Mapping Scheme}
\label{subsubsec:aim1mapping}

Hand in hand with the above, I will also need to formally define and test a practical mapping scheme, both for mapping during the construction of a reference structure, and for mapping short reads (and upcoming long reads) to that reference structure. In this area my prior work has proven more fruitful. I have prototyped the (general) left-right exact match mapping scheme of Subsection~\ref{subsec:mapping}. When used with the greedy merging scheme, left-right exact match mapping can produce a reasonable-looking reference structure out of the GRCh38 MHC region and its alt loci. However, that reference structure is still not all that great; there are large regions which are not merged due to mismatches or other small differences between the sequences being merged.
...
\subsubsection{Construct a Mathematics of Variants}
\label{subsubsec:aim1math}
...
\label{subsubsec:aim1deliverables}
...
\subsection{Specific Aim \#2: Engineer Scalable Software Tools}
\label{subsec:aim2}
...
\subsubsection{Build an Aligner for GRCh38 with Alt Loci}
\label{subsubsec:aim2aligner}
...
\subsubsection{Incorporate Additional Variation}
\label{subsubsec:aim2variation}
...
\subsubsection{Provide Reference Hierarchy APIs}
\label{subsubsec:aim2api}
...
\subsubsection{Deliverables} 
\label{subsubsec:aim2deliverables}
...
\subsection{Specific Aim \#3: Discover Biologically Relevant Variation}
...
\subsubsection{Apply Reference Structures to NOTCH2NL}
\label{subsubsec:aim3notch}
...
\subsubsection{Realign Existing Datasets}
\label{subsubsec:aim3realign}
